                            ...
                            <div class="card-body table-responsive">
                                <table class="table table-bordered table-hover datatable">
                                    <thead>
                                        <tr>
                                            <th>#</th>
                                            <th>@lang('admin.specialty')</th>
                                            <th>@lang('admin.subspecialty')</th>
                                            <th>@lang('admin.Updated')</th>
                                            @canany(['update specialties', 'delete specialties'])
                                                <th>@lang('admin.Action')</th>
                                            @endcanany
                                        </tr>
                                    </thead>
                                    <tbody>
                                        @foreach ($specialties as $i)
                                            <tr>
                                                <td>{{ $loop->iteration }}</td>
                                                <td><a href="{{ route('subspecialties.index') }}">{{ $i->name }}</a></td>
                                                <td>

                                                    <ul>
                                                        @foreach (Modules\Specialties\Entities\Subspecialty::where('specialty_id',$i->id)->get() as $sp)
                                                        <li>{{ $sp->name }}</li>
                                                        @endforeach
                ...
                <div class="modal-body">
                    <form action="{{ route('specialties.store') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="input-group">
                            ...
                            <div class="input-group">
                                <input type="text" id="specialty-dropdown"  name="name" class="form-control @error('name') is-invalid @enderror" placeholder="Name" name="name" value="{{ old('name') }}">
                                @error('name')
    ...
    <div class="modal fade" id="modal-edit">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('specialties.update') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method("PUT")
                        <div class="input-group">
                            <label>@lang('admin.Name')</label>
    ...
    <div class="modal fade" id="modal-delete">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <form action="{{ route('specialties.destroy') }}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('DELETE')
